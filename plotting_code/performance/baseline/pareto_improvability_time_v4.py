...
PLOT_NAME = "pareto_improvability_time_v4.png"

# TODO: Adapt title and labels
PLOT_TITLE = ""
X_LABEL = "Time Delay vs. Random (%)"
Y_LABEL = "Performance Improvement vs. Random (%)"
...
def calculate_relative_performance(df):
    ...
    df_avg = (
        df.groupby(["tid", "feature_selection_method"])[["metric_error", "feature_selection_fit_time"]]
        .mean()
        .reset_index()
    )
    ...
    random_time_baseline = (
        random_errors.groupby("tid")["feature_selection_fit_time"]
        .mean()
        .reset_index()
        .rename(columns={"feature_selection_fit_time": "random_time"})
    )

    # 3. Merge best error back
    df_merged = df_avg.merge(random_performance_baseline, on="tid", how="left")
    df_merged = df_merged.merge(random_time_baseline, on="tid", how="left")
