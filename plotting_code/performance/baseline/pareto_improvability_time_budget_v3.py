...
PLOT_NAME = "pareto_improvability_time_budget_v3.pdf"
...
def plot(df):
    ...
    for idx_budget, budget in enumerate(budgets):
        ...
        # Use modulo just in case there are more budgets than predefined colors
        color = colors[idx_budget % len(colors)]
        ...
        # 2. Draw connecting line for Pareto front
        ax.plot(
            pareto["mean_time"],
            pareto["mean_score"],
            color=color,
        ...
        # 3. Prepare text labels for PARETO points ONLY
