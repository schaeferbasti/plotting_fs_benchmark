...
from utils.scaling import z_score_scale, median_max_scale, tabarena_normalization
...
def calculate_relative_performance(df):
    ...
    # Use raw metric_error (lower is better)
    random_scores = df[df["feature_selection_method"] == "Random"][
        ["tid", "metric", "metric_error"]
    ]
    random_scores = random_scores.rename(columns={"metric_error": "random_error"})

    df = df.merge(random_scores, on=["tid"], how="left")

    # Math: (Random Error - Method Error) * 100
    # Positive result = Model has less error than Random (Improvement)
    df["improvability"] = (df["random_error"] - df["metric_error"]) * 100

    # Aggregate
    agg_df = df.groupby("feature_selection_method")["improvability"].agg(["mean", "std"]).reset_index()
