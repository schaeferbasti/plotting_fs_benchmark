...
from utils.scaling import min_max_scale, tabarena_normalization

FILE_NAME = "results_per_split.csv"
PLOT_NAME = "relative_performance_v2.png"
PLOT_TITLE = "Score Improvement in comparison to Random"
...
def calculate_relative_performance(df):
    ...
    random_errors = df[df["feature_selection_method"] == "Random"][
        ["tid", "metric", "normalized_score"]
    ]
    random_errors = random_errors.rename(columns={"normalized_score": "random_score"})
    ...
    df["score_improvement"] = df["normalized_score"] - df["random_score"]

    # 5. Aggregate mean improvement and std per feature selection method
    agg_df = df.groupby("feature_selection_method")["score_improvement"].agg(["mean", "std"]).reset_index()
    agg_df.columns = ["feature_selection_method", "mean_improvement", "std_improvement"]
...
def plot_relative(df):
    ...
    agg_df = agg_df.sort_values("mean_improvement", ascending=False)

    methods = agg_df["feature_selection_method"].values
    scores = agg_df["mean_improvement"].values
    stds = agg_df["std_improvement"].values
