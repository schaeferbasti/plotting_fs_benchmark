...
PLOT_NAME = "performance_per_model_v2.png"

# TODO: Adapt title and labels
PLOT_TITLE = "Performance per Downstream Model (robust scaling)"
X_LABEL = "Feature Selection Method"
Y_LABEL = "Score (0 = Median Method, >0 is Better)"
...
def calculate_robust_performance(df):
    ...
    # 3. Robust Scale the performance per dataset/metric
    def robust_scale(group):
        median_val = group.median()
        q75, q25 = group.quantile(0.75), group.quantile(0.25)
        iqr = q75 - q25

        if iqr == 0:
            # If all methods perform identically, variance is 0
            return pd.Series(0.0, index=group.index)

        return (group - median_val) / iqr

    df_collapsed["scaled_score"] = df_collapsed.groupby(group_cols)["performance"].transform(robust_scale)

    return df_collapsed


def plot(df):
    df_scaled = calculate_robust_performance(df)
    ...
    # Draw a baseline at 0 (The Median Performance)
    ...
    print(f"✅ Robust performance per model plot saved to {out}")
